...
fn polynomial_complex() {
	...
	let b = Complex::i() * Complex::<f32>::from(2.0);
...
fn monomial() {
	// P(x) = i*x²
	let poly_1 = Polynomial::new_monomial(Complex::<f32>::i(), 2);

	assert_eq!(poly_1.apply(Complex::<f32>::from(2.0)), Complex::i() * Complex::<f32>::from(4.0));
}
