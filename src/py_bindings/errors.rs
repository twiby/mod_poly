...
impl ErrorTypeToString for polynomial::ModularArithmeticError {
    fn str() -> String {
...
impl From<polynomial::ModularArithmeticError> for pyo3::PyErr {
    fn from(e: polynomial::ModularArithmeticError) -> Self {
        match e {
            polynomial::ModularArithmeticError::ModulusMismatched(s) => {
                py_value_error::<polynomial::ModularArithmeticError>(&s)
            }
            polynomial::ModularArithmeticError::DegreeAboveModulus(s) => {
                py_value_error::<polynomial::ModularArithmeticError>(&s)
            }
        }
    }
...
/// Translate ModularArithmeticError name
impl ErrorTypeToString for matrix::MatrixError {
    fn str() -> String {
        "MatrixError: ".to_string()
    }
}
/// Immplementing the From trait for errors Rust -> Python
impl From<matrix::MatrixError> for pyo3::PyErr {
    fn from(e: matrix::MatrixError) -> Self {
        match e {
            ...
            matrix::MatrixError::WrongInputArraySize(s) => {
                py_value_error::<matrix::MatrixError>(&s)
            }
            matrix::MatrixError::UncompatibleMatrixShapes(s) => {
                py_value_error::<matrix::MatrixError>(&s)
            }
