...
use crate::polynomial::{ModularArithmeticError, ModularArithmeticPolynomial};
...
impl<T: MatrixInput> Matrix<T> {
    ...
    pub fn new_empty(rows: usize, cols: usize, default: T) -> MatrixResult<T> {
        ...
        Ok(Self {
            arr: vec![default.clone(); rows * cols],
            cols: cols,
            rows: rows,
        })
    }
    ...
    pub fn new(arr: Vec<T>, x: usize, y: usize) -> MatrixResult<T> {
        ...
        if arr.len() != x * y {
            return Err(MatrixError::WrongInputArraySize(format!(
                "Wrong input size: {} instead of {}",
                arr.len(),
                x * y
            )));
        }
    ...
    pub fn check_idx(&self, x: usize, y: usize) -> Result<(), MatrixError> {
        if x >= self.rows {
            return Err(MatrixError::OutOfBoundsIndex(format!(
                "x index too high: {} for size {}",
                x, self.rows
            )));
        } else if y >= self.cols {
            return Err(MatrixError::OutOfBoundsIndex(format!(
                "y index too high: {} for size {}",
                y, self.cols
            )));
        } else {
            Ok(())
    ...
    fn add(self, other: &'a Matrix<T>) -> MatrixResult<T> {
        if self.shape() != other.shape() {
            return Err(MatrixError::UncompatibleMatrixShapes(format!(
                "Uncompatible matrix shapes for addition, {:?} and {:?}",
                self.shape(),
                other.shape()
            )));
        }
...
impl<'a, T: Number> Add for &'a Matrix<ModularArithmeticPolynomial<T>> {
    ...
    fn add(
        ...
        Ok(Matrix::<ModularArithmeticPolynomial<T>> {
            arr: vec,
            cols: self.cols,
            rows: self.rows,
        })
